...
class BearOauth2UserService {
    public static function createOrUpdateFromCodeWithOidc(string $code, BearOauth2Client $client, string $redirectUri): BearOauth2User {
        ...
        $token = OidcToken::fromJwt(jwt: $resp['id_token'], client: $client);
        ...
        $user->oauth2_scope = is_array($resp['scope']) ? implode(' ', $resp['scope']) : $resp['scope'];
        $scope_array = is_array($resp['scope']) ? $resp['scope'] : explode(' ', $resp['scope']);
